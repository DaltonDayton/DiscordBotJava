          ...
          channel.sendMessage("Pick one:\n")
              ...
              .queue((message -> {
                ...
                waiter.waitForEvent(
                    ...
                    (e) -> {
                      String selectedEmoji = EmojiParser.parseToAliases(e.getReactionEmote().getEmoji());
                      switch (selectedEmoji) {
                        case ":one:" -> selectedEmojiIndex.set(0);
                        case ":two:" -> selectedEmojiIndex.set(1);
                        case ":three:" -> selectedEmojiIndex.set(2);
                        case ":four:" -> selectedEmojiIndex.set(3);
                        case ":five:" -> selectedEmojiIndex.set(4);
                      }

                      channel.sendMessageFormat("Adding %s to the queue.", tracks.get(selectedEmojiIndex.get()).getInfo().title).queue();
